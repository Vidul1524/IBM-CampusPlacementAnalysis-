...
def pred():
    try:
        ...
        # Original input data
        input_data = [[ssc_p/100, hsc_p/100, degree_p/100, gender, workex, etest_p/100, specialisation, mba_p/100]]

        # Apply the same preprocessing as in Jupyter Notebook
        scaled_input_data = preprocessing.transform(input_data)

        op = model.predict(scaled_input_data)
        logging.debug("Model output: %s", str(op))

        result = "Placed" if op[0] > 0.5 else "Not Placed"
